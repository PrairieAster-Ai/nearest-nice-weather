...
export function WeatherResultsComponent({ results, loading = false }: WeatherResultsProps) {
  ...
  const { isMobile, isTablet } = deviceBreakpoints
  
  const [showCompletionIndicator, setShowCompletionIndicator] = useState(false)
  const [fadeOut, setFadeOut] = useState(false)
  const [animationClass, setAnimationClass] = useState('prairie-aster-bounce')

  // Array of available animation classes
  const animationVariants = [
    'prairie-aster-bounce',
    'prairie-aster-pulse', 
    'prairie-aster-float',
    'prairie-aster-spin',
    'prairie-aster-elastic',
    'prairie-aster-swing'
  ]

  // Randomize animation when loading starts
  useEffect(() => {
    if (loading) {
      const randomIndex = Math.floor(Math.random() * animationVariants.length)
      const selectedAnimation = animationVariants[randomIndex]
      setAnimationClass(selectedAnimation)
      // console.log('🎭 Selected animation:', selectedAnimation)
    }
  }, [loading])

  // Handle fade-out effect when loading completes
  useEffect(() => {
    if (!loading && results.length > 0) {
      setShowCompletionIndicator(true)
      setFadeOut(false)
      
      // Start fade-out after 1 second, complete over 3 seconds
  ...
  return (
    <div className="relative h-[32rem]"> {/* Increased from h-96 (24rem) to h-[32rem] */}
      <WeatherMap results={results} loading={loading} />
      
      {/* Large loading overlay on map */}
      {loading && (
        <div 
          style={{
            position: 'absolute',
            ...
            right: 0,
            bottom: 0,
            backgroundColor: 'rgba(255, 255, 255, 0.34)', // Reduced opacity by 60%
      ...
      {/* Completion indicator with fade-out */}
      {showCompletionIndicator && (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{ fontSize: '18px', fontWeight: '500', color: '#1f2937' }}>{results.length} locations found</span>
          </div>
        </div>
      )}
    </div>
